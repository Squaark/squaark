...
### Added
- Page builder — schema-driven sections + a new **Hero** section. Each section type now declares its settings once (in a central registry), and the page editor's form, defaults and validation are generated from that declaration rather than hand-coded per type — so adding a section is a schema entry plus a theme partial, nothing more. Section data is whitelisted and defaulted **server-side** on save (unknown types/fields never reach the DB). The first new section on this foundation is **Hero** (heading, subheading, background image, button, left/centre alignment, plus **background and text colour**), styled in both bundled themes. Colour fields offer a picker that includes your live **site colours** (Highlight/Primary/Background) — chosen as theme tokens so the hero follows a later brand-colour change rather than freezing a hex — with a custom-colour option; values are validated server-side so they can't break out of the inline style. Existing pages and their sections are unaffected.
