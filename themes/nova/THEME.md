...
1. **`partials/sections/<type>.hbs`** — one partial per section type, rendered
   with the section's settings as its context (`{{heading}}`, `{{#each images}}`,
   …). This theme ships partials for all 17 types (hero, featured_products,
   gallery, testimonials, logo_row, slideshow, faq, newsletter, video, map,
   spacer, text, image, image_text, cta, columns — `reusable` is expanded
   server-side). A missing partial just renders nothing.
