...
async function createProduct(
  ...
  execute(
    `INSERT INTO products (id, title, slug, description, vendor, tags_text, published, seo_title, seo_description, free_shipping, is_digital, tax_band_id, available_from, available_until, allow_preorder)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
    [productId, title.trim(), slug.trim(), description || null, vendor || null, tags_text || '', published === '1' ? 1 : 0,
     ...
     is_digital === '1' ? 1 : 0, tax_band_id, available_from, available_until, allow_preorder],
  );
  execute(
...
async function updateProduct(
  ...
  const allow_preorder = req.body.allow_preorder === '1' ? 1 : 0;
  execute(
    `UPDATE products SET title=?, slug=?, description=?, vendor=?, tags_text=?, published=?, seo_title=?, seo_description=?, free_shipping=?, is_digital=?, tax_band_id=?, available_from=?, available_until=?, allow_preorder=?, updated_at=datetime('now') WHERE id=?`,
    [title, slug, description || null, vendor || null, tags_text || '', published === '1' ? 1 : 0,
     seo_title || null, seo_description || null, free_shipping === '1' ? 1 : 0,
     is_digital_update === '1' ? 1 : 0, tax_band_id_update, available_from, available_until, allow_preorder, id],
  );
